...
// Create a new rubric
    ...
    public static Double getAverageCriterion(int studentGrade, Rubric rubric) {
        double sum = 0; // Average will need decimal
        int questionNum = 0;
        for (int j = 0; j < rubric.getGrades().size(); j++) {
            StudentGrade sGrade  = rubric.getGrades().get(j);
            sum += sGrade.getAnswers().get(surveyQuestion);
            questionNum++;
        }
        sum = sum / questionNum;
        return sum;
