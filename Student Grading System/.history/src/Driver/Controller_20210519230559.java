...
public class Controller {
    ...
    switch (scan.next()){
        ...
        case("2"):
            ...
            getARubric(rubricWanted,r);
            if(r != null){
                System.out.println("Rubric found " + " " +  r.getRubricName()); 
            }
            showMenu();
            break;
        case("3"):
...
// Create a new rubric
    public Boolean createRubric(String rubricTobeCreated){
        ...
        {
            System.out.println("Rubric name on creation " + r.getRubricName());
        ...
        public Boolean createCriterion(String criterionToBeCreated, String rubricname)
        {
            ...
            //3. Else add it
            Rubric r = getARubric(rubricname, rubricReturned);
...
// 
    public Rubric getARubric(String nameOfRubric, Rubric rubricReturned)
    {
        rubricReturned = null;
...
//	public StudentGrade createStudentGrade(Rubric rub)
	//{		
		//Creation of a new survey response object
//		StudentGrade sg = new StudentGrade(rub.getCriteria());
//		return sg;
	//}
    ...
    public Boolean addStudentGrade(String criteriaName, String rubricName, String nameOfStudent,int gradeOfStudent)
    {
        ...
        for (Rubric r : listOfRubrics)
        {
            if(r.getRubricName() == rubricName)
            {
                listOfCriteria = r.getCriteria();

                for (Criterion c : listOfCriteria)
                {
                    if(c.getCriteriaName() == criteriaName)
                    {
